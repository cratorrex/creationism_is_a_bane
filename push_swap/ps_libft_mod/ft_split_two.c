...
//population increase
static char	**alloc_arr(char const *s, char c1, char c2, char **array)
{
	size_t	size;
	size_t	prev;
	size_t	word;
	...
	while (s[prev])
	{
		while (s[prev] == c1 || s[prev] == c2)
			prev++;
		while ((s[prev + size] != c1 && s[prev + size] != c2)
			&& s[prev + size] != '\0')
			size++;
...
static char	**alloc_big(char const *s, char c1, char c2)
{
	size_t	i;
	size_t	dcount;
	char	**big;
...
char	**ft_split_two(char const *s, char c1, char c2)
{
	...
	array = alloc_big(s, c1, c2);
	if (array)
	{
		array = alloc_arr(s, c1, c2, array);
		if (array == NULL)
			return (NULL);
		return (array);
	}
