...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/05/03 17:27:08 by thtay             #+#    #+#             */
/*   Updated: 2026/05/03 17:27:10 by thtay            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
///
/// Yes... I literally copied my gnl bonus c here and made changes to it...
///
...
static char	*gnl_read_loop(char *fdstr, int fd, char *buffer, char *limit)
{
	ssize_t		r_idx;
...
char	*get_next_line_hd(int fd, char *limit)
{
	...
	if (stash[fd] == NULL)
		stash[fd] = gnl_strdup("");
	stash[fd] = gnl_read_loop(stash[fd], fd, buffer, limit);
	free(buffer);
	if (stash[fd] == NULL)
		return (NULL);
